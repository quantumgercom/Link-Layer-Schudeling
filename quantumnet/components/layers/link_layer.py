...
class LinkLayer:
    ...
    def purification_scheduling(self, alice_id: int, bob_id: int, purification_type: str, rounds: int = 1, Policy: int = 1, Try: int = None):
        ...
        valid_purification_types = ['symmetric', 'pumping', 'symmetric_proposal', 'pumping_proposal']
        ...
        elif purification_type_corrected == 'symmetric_proposal':
            if Policy == 1:
                # Política padrão: tenta apenas uma vez
                return self.purification_symmetric_proposal(alice_id, bob_id, rounds)
            elif Policy == 2:
                # Política de múltiplas tentativas
                for attempt in range(Try):
                    if self.purification_symmetric_proposal(alice_id, bob_id, rounds):
                        return True
                    self.logger.log(f"Tentativa {attempt + 1}/{Try} falhou para purificação simétrica com proposta.")
                return False
            
        elif purification_type_corrected == 'pumping_proposal':
            if Policy == 1:
                # Política padrão: tenta apenas uma vez
                return self.purification_pumping_proposal(alice_id, bob_id, rounds)
            elif Policy == 2:
                # Política de múltiplas tentativas
                for attempt in range(Try):
                    if self.purification_pumping_proposal(alice_id, bob_id, rounds):
                        return True
                    self.logger.log(f"Tentativa {attempt + 1}/{Try} falhou para purificação por bombardeamento com proposta.")
                return False
        return False
    ...
    #TODO:Proposta
    def purification_symmetric_proposal(self, alice_id: int, bob_id: int, rounds: int) -> bool:
        """
        ...
        auxiliary_used = False  # Indica se a purificação auxiliar já foi usada como substituição
        ...
        for round_num in range(rounds):
            ...
            while i < num_eprs_necessarios:
                ...
                if not success:
                    ...
                    if not auxiliary_used:  # Se ainda não foi usada, chama a purificação auxiliar
                        aux_eprs = auxiliary_purification()
                        auxiliary_used = True  # Marca que a purificação auxiliar foi utilizada
                        self.logger.log(f"Purificação auxiliar gerou {len(aux_eprs)} novos EPRs auxiliares.")
                    else:  # Se já foi utilizada, encerra o código
                        self.logger.log(f"Erro: Não há backups suficientes para substituir os pares falhados. Encerrando purificação.")
                        self.last_error = "Falha devido à insuficiência de backups auxiliares."
                        return False
    ...
    #TODO:Proposta
    def purification_pumping_proposal (self, alice_id: int, bob_id: int, rounds: int) -> bool:
        """
    ...
    def round_estimates(self, alice_id: int, bob_id: int, target_fidelity: float = 1.0, purification_type: str = None, decoherence_factor: float = 0.01):
        """
        ...
        def calculate_for_type(purification_type):
            ...
            while current_fidelity < target_fidelity and rounds < MAX_ROUNDS:
                ...
                if purification_type == "symmetric":
                    ...
                    if rounds == 1:
                        # No primeiro round, dois pares EPR base são necessários
                        num_eprs_base_needed = 2
                    else:
                        # Nos rounds subsequentes, um novo par é criado e combinado com o f' anterior
                        num_eprs_base_needed = 1
                else:
                ...
                f1 = current_fidelity
                if purification_type == "symmetric":
                    f2 = f1  # Ambos os pares têm a mesma fidelidade no caso simétrico
                elif purification_type == "pumping":
                    if rounds == 1:
                        f2 = f1  # No primeiro round, ambos os pares têm a mesma fidelidade
                    else:
                        f2 = initial_fidelity  # Em rounds subsequentes, um novo par com fidelidade inicial é usado
                ...
                # Aplica decoerência apenas em rounds intermediários
                if rounds > 1 and current_fidelity < target_fidelity:  # Não aplica no primeiro e no último round
                    current_fidelity -= current_fidelity * decoherence_factor
